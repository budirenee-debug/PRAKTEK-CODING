...
- **Klaim Garansi REVISI-2 (belum uji browser):** semua tab status + Proses urut terbaru dulu (`updated_at` kini ikut ke frontend — sblmnya sort-nya ngaco karena field-nya null); Sukses → muncul di Sudah Diambil DAN tab Garansi selama masa garansi aktif (filter `garansiAktif`); klaim backend terima status Sukses/Selesai juga.
- **Tgl Pengambilan (belum uji browser):** kolom `diambil_at` auto-isi persis saat status jadi Sukses/Sudah Diambil (+migrasi backfill dari `updated_at`); badge hijau "📥 Diambil 23 Sep 2026 • 14.30" di kartu Sudah Diambil.
- **Pendapatan Sukses = Sukses saja (belum uji browser):** tab Pendapatan (tabel, rekap 7 hari, sparepart, profit, badge, export CSV) hanya hitung status Service Sukses — Bisa Diambil tidak ikut agar angka real.
- **Detail read-only tab terminal (belum uji browser):** tombol Detail di Bisa Diambil / Sudah Diambil / Failed / Garansi / Pendapatan buka modal info saja (tanpa kotak edit, tanpa Assign/Simpan/Ubah Estimasi). Proses & Semua Service tetap bisa edit.
- **Filter teknisi per tab (belum uji browser):** dropdown 👨‍🔧 di 7 tab (Semua, Proses, Bisa, Sudah, Failed, Garansi, Pendapatan), opsi dari anggota toko.
- **Batas teknisi (E2E lolos, belum uji browser):** teknisi hanya lihat service miliknya (cocok username/nama) + Menunggu Teknisi; pelanggan 403 + menu disembunyikan; ubah/oper milik orang ditolak backend; inventory & dashboard tetap; assignment teknisi terbatas ke diri sendiri. Data lama yg nama teknisi beda jauh dari username perlu re-assign sekali.
- **Foto profil (E2E lolos):** upload di Atur Profil, kompres otomatis max 256px JPG q70 (maks 5MB, butuh Pillow), tampil sidebar+laporan; folder avatars di-ignore git.
- **Template WA per status (E2E lolos):** `wa_templates` per toko (masuk/sukses/gagal/diambil/garansi/umum, variabel {nama}{device}{invoice}{keluhan}{toko}{biaya}{teknisi}{status}{garansi_dari}); view Pengaturan Toko (owner/admin); tombol WA Proses pakai template service_masuk.
- **Alasan gagal wajib (belum uji browser):** modal alasan TIDAK/gagal kini muncul dari tab mana pun (sblmnya hanya dari Proses) — kartu Failed tampil "📋 Alasan:" utk evaluasi. `?v=multistore-gagal-alasan`.
