...
def create_sparepart(payload: schemas.SparepartCreate, db: Session = Depends(get_db)):
    # cek duplikat nama
...
def update_sparepart(sp_id: int, payload: schemas.SparepartUpdate, db: Session = Depends(get_db)):
...
def delete_sparepart(sp_id: int, db: Session = Depends(get_db)):
...
def pakai_sparepart(sp_id: int, qty: int = Query(1, ge=1, description="Jumlah pakai"), db: Session = Depends(get_db)):
...
def create_alat(payload: schemas.AlatCreate, db: Session = Depends(get_db)):
...
def update_alat(alat_id: int, payload: schemas.AlatUpdate, db: Session = Depends(get_db)):
    if payload.nama:
