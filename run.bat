...
if errorlevel 1 (
  echo [WARN] fastapi/uvicorn belum terimport G현 coba install butuh internet...
  ...
  if errorlevel 1 (
    echo [WARN] pip install gagal G현 cek internet / proxy. Coba manual:
...
:: Jika port busy (pindah jaringan), tunnel lama kemungkinan putus G현 kill & restart agar publik kembali online
if "%PORT_BUSY%"=="1" if not "%MODE%"=="none" (
  echo [INFO] Port busy + pindah jaringan terdeteksi G현 restart tunnel lama...
